...
INSTALLED_APPS = [
    ...
    "cloudinary_storage",  # must come before staticfiles; inert until CLOUDINARY_* env vars are set
...
# Database
# SQLite by default (local dev / free-tier friendly). Set DATABASE_URL to
# point at Postgres (e.g. Supabase/Neon) for production without code changes.
...
# an ephemeral filesystem (Render, Railway) local uploads are lost on redeploy, so
# set the CLOUDINARY_* env vars in production.
MEDIA_URL = "media/"
